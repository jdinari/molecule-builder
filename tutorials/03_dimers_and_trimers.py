...
Covers:
  1. Di-mu-hydroxo and di-mu-formate Ni(II) dimers
...
Structures that did NOT work at the time of writing and why:
  ...
  * Ru3(mu-CO) triangular trimers -- bridging CO on a triangular Ru3
...
print("Ni2(mu-OH)2(H2O)4:")
info(mol)


# -- 2. Di-mu-formate dimer ----------------------------------------------------
#
# [Ni2(mu-HCOO)2(H2O)6]
...
#
# 4 bridging formates, no terminals.  Short Ni-Ni distance (~3.98 Angstrom).
...
print(f"\nNi2(mu-HCOO)4 paddle-wheel:  Ni-Ni = {ni_ni:.3f} Angstrom  charge={mol.charge}")


# -- 4. Heteroleptic dimer: asymmetric terminals -------------------------------
#
# [Ni2(mu-HCOO)4(H2O)] -- water on one Ni only, the other is bare.
...
print(f"\nNi2(mu-HCOO)4(H2O) heteroleptic:  {mol.formula}  charge={mol.charge}")
cn_each = []
...
# The +/-35deg tilt scheme ensures all O***O distances > 2.2 Angstrom.
...
print(f"\nNi3(mu-HCOO)6 triangular:  {mol.formula}  charge={mol.charge}")
print(f"  Ni-Ni distances: {[round(d, 3) for d in ni_ni_dists]} Angstrom  (equilateral)")


# -- 7. Heteroleptic Ni3 -- water on one metal only ----------------------------
#
# [Ni3(mu-HCOO)6(H2O)]: H2O on metal-0 only, metals 1 and 2 are bare.
...
print(f"\nNi3(mu-HCOO)6 + H2O on Ni0:  {mol.formula}  charge={mol.charge}")
