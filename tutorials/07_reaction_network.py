"""
Tutorial 07 -- Isodesmic Reaction Network and DeltaG Screening
===========================================================
...
3.  Compute DeltaG for every node with xTB thermochemistry.
4.  Screen for the most accessible reactions (DeltaG < threshold).
...
This is the canonical isodesmic substitution:
  ...
  * The references are HCOOH and H2O, both neutral  ->  no charged free species.
  * DeltaG is fully reliable at the xTB level in gas phase.
...
print("\n=== Step 3: The formic acid route (HCOOH + [Ni-OH] -> [Ni-HCOO] + H2O) ===")
hcooh_edges = [
...
# -- 5. Compute xTB DeltaG ---------------------------------------------------------
print("\n=== Step 4: xTB thermochemistry (DeltaG at 298.15 K) ===")
print("  (this takes ~2 min for a small inventory)")
try:
    net.compute_energies(
        backend        = "xtb",
        compute_thermo = True,    # <- DeltaG is the default
        T              = 298.15,
        P              = 101325.0,
...
if has_energies:
    print("\n=== Step 5: Screen reactions (DeltaG <= 1.0 eV) ===")
    print()

    hits = net.screen(max_dE=1.0, use_gibbs=True, require_energy=True)
    print(f"  {len(hits)} reaction(s) with DeltaG <= 1.0 eV:")
    ...
    print()
    print("  Formic acid substitutions (HCOOH + [Ni-OH] -> [Ni-HCOO] + H2O):")
    hcooh_hits = [
    ...
    if hcooh_hits:
        ...
        print("    (none within DeltaG <= 1.0 eV threshold -- try increasing max_dE)")
...
try:
    ...
    fig = net.plot(
        ...
        title         = "Ni coordination network (isodesmic, DeltaG eV)",
        show_broken   = True,
    )
...
print()
print("Key takeaways:")
print("  * HCOOH + [Ni-OH] -> [Ni-HCOO] + H2O is the formic acid route.")
print("    It is isodesmic (DeltaG fully reliable) and already in the network.")
print("  * Broken structures are excluded from reactions and written to out/broken/.")
print("  * screen(max_dE=X) returns accessible reactions sorted by DeltaG.")
